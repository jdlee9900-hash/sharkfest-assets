...
// Lets the primary account holder add/change the partner email for shared access.
function PartnerEmailCard({ registration }: { registration: Registration & { partner_email?: string | null } }) {
  const [email, setEmail]     = useState(registration.partner_email ?? '')
  const [saved, setSaved]     = useState(registration.partner_email ?? '')
  const [saving, setSaving]   = useState(false)
  const [error, setError]     = useState('')
  const [done, setDone]       = useState(false)
  ...
  return (
    <div className="mb-card">
      ...
      <p className="mb-camp-intro">
        Add a second email address to share this booking. They&apos;ll receive an invite
        and can log in to view payments and manage camp preferences — handy for partners
        or other people in your party.
      </p>
    ...
    <div className="mb-wrap">
      ...
      {!registration ? (
        ...
        <>
          ...
          {!isPartner && (
            <PartnerEmailCard registration={registration as Registration & { partner_email?: string | null }} />
          )}
